#include "fonctions.h"

void		look(int salle)
{
  if(salle == 1)
    {
      my_putstr("#Fonction manquante : texte en cours d'écriture.\n\n");
    }
  if(salle == 2)
    {
      my_putstr("#Fonction manquante : texte en cours d'écriture.\n\n");
    }
  if(salle == 3)
    {
      my_putstr("#Fonction manquante : texte en cours d'écriture.\n\n");
    }
  if(salle == 4)
    {
      my_putstr("#Fonction manquante : texte en cours d'écriture.\n\n");
    }
  if(salle == 5)
    {
      my_putstr("#Fonction manquante : texte en cours d'écriture.\n\n");
    }
  if(salle == 6)
    {
      my_putstr("#Fonction manquante : texte en cours d'écriture.\n\n");
    }
  if(salle == 7)
    {
      my_putstr("\n#Fonction manquante : texte en cours d'écriture.\n\n");
    }
  if(salle == 8)
    {
      my_putstr("\n\nJe suis dans l'infirmerie. Le sol est couvert de sang.\n");
      if(CARTE_ACCES == 0)
	{
	  my_putstr("Si je n'etais pas autant ecoeuree, je fouillerai peut etre le corps du Sergent Colas.\n\n");
	}
      else
	{
	  my_putstr("Il y a une porte la derriere, peut etre une sorte de reserve ?\n\n");
	}
    }
  if(salle == 9)
    {
      my_putstr("\nJe suis dans la grande salle. Il m'est difficile de rester la, la vision de ces cadavres est insupportable...\n\n");
    }
  if(salle == 10)
    {
      my_putstr("#Fonction manquante : texte en cours d'écriture.\n\n");
    }
  if(salle == 11)
    {
      my_putstr("\nJe suis dans la reserve de l'infirmierie. La plupart des appareils qui sont la sont endommages, dommage.\n");
      my_putstr("\nJ'espere trouver quelques medicaments dans ce bazar...\n\n");
    }
  if(salle == 12)
    {
      my_putstr("\nJe suis dans un grand couloir. il n' a rien a faire ici a part s'assoir et pleurer...");
      my_putstr("\nMais ne vous inquietez pas je ne vais pas mettre a pleurer... Pas tout de suite.\n");
      my_putstr("D'ici je peux aller dans toutes les directions.\n\n");
    }
  if(salle == 13)
    {
      my_putstr("#Fonction manquante : texte en cours d'écriture.\n\n");
    }
  if(salle == 14)
    {
      my_putstr("#Fonction manquante : texte en cours d'écriture.\n\n");
    }
  if(salle == 15)
    {
      my_putstr("#Fonction manquante : texte en cours d'écriture.\n\n");
    }
  if(salle == 16)
    {
      my_putstr("#Fonction manquante : texte en cours d'écriture.\n\n");
    }
  if(salle == 17)
    {
      my_putstr("#Fonction manquante : texte en cours d'écriture.\n\n");
    }
  if(salle == 18)
    {
      my_putstr("#Fonction manquante : texte en cours d'écriture.\n\n");
    }
  if(salle == 19)
    {
      my_putstr("#Fonction manquante : texte en cours d'écriture.\n\n");
    }
  if(salle == 20)
    {
      my_putstr("#Fonction manquante : texte en cours d'écriture.\n\n");
    }
  if(salle == 21)
    {
      my_putstr("#Fonction manquante : texte en cours d'écriture.\n\n");
    }
  if(salle == 22)
    {
      my_putstr("#Fonction manquante : texte en cours d'écriture.\n\n");
    }
  if(salle == 23)
    {
      my_putstr("#Fonction manquante : texte en cours d'écriture.\n\n");
    }
  if(salle == 24)
    {
      my_putstr("#Fonction manquante : texte en cours d'écriture.\n\n");
    }
}

  
  
